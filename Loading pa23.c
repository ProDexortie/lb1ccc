...
int receive_any(void * self, Message * msg) {
    ...
    for (;;) {
        for (local_id from = 0; from < ctx->nprocs; ++from) {
            ...
            int fd = get_rd_fd(ctx, from);
            if (fd < 0) continue;
            // peek header: try to read; if header available, we must consume exact stream, so we cannot "peek".
            // We'll try to read; on EAGAIN we'll retry other fds.
...
static void build_pipes(Ctx* root, int nchildren) {
    ...
    for (int i = 0; i < nprocs; ++i) {
        for (int j = i + 1; j < nprocs; ++j) {
            ...
            // j -> i uses p2
            root->ch[j][i].wr = p2[1];
            root->ch[j][i].rd = root->ch[j][i].rd; // already set
            root->ch[i][j].rd = p2[0];
            // Note: we won't use ch[i][i]
        }
    }
}
    ...
    for (;;) {
        ...
        if (receive_any(parent_data, &rcv) == 0) {
            if (rcv.s_header.s_type == ACK) {
                // we do not know who sent by header alone; but receive_any consumed from some FD
                // In this simple implementation, we accept any ACK; robustly we should ensure from dst.
                break;
            }
            // ignore other types
        }
    ...
    while (got_started < need_started) {
        ...
        if (receive_any(ctx, &m) == 0) {
            if (m.s_header.s_type == STARTED) got_started++;
                ...
                // queue other messages for later by handling immediately:
                if (m.s_header.s_type == TRANSFER) {
                    TransferOrder ord;
                    memcpy(&ord, m.s_payload, sizeof(ord));
                    if (ord.s_dst == ctx->id) {
                        // incoming to me should not happen before I finish STARTED exchange in a well-behaved test,
    ...
    for (;;) {
        ...
        switch (m.s_header.s_type) {
            case TRANSFER: {
                ...
                memcpy(&ord, m.s_payload, sizeof(ord));
                if (ord.s_src == ctx->id) {
                    // I am source: decrease and forward
                    acc.balance -= ord.s_amount;
                    hist_on_change(&acc);
                    log_transfer_out(ctx, ord.s_amount, ord.s_dst);
                    Message fwd; msg_init(&fwd, TRANSFER, &ord, (uint16_t)sizeof(ord));
                    ...
                    // I am destination: increase and ACK to parent
                    acc.balance += ord.s_amount;
                    hist_on_change(&acc);
                    log_transfer_in(ctx, ord.s_amount, ord.s_src);
                    Message ack; msg_init(&ack, ACK, NULL, 0);
            ...
            case DONE: {
                ...
                if (stop_received && done_from_others >= need_done_from_others) {
                    // finalize history to current time
                    hist_fill_to(&acc, get_physical_time());
                    // send BALANCE_HISTORY to parent, compact length
                    Message histmsg;
